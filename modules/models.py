...
def load_model(model_name, loader=None):
    ...
    if loader.lower().startswith('exllama') or loader.lower().startswith('tensorrt') or loader in ['llama.cpp', '1Cat-vLLM']:
...
def vllm_loader(model_name):
    from modules.vllm import vLLMServer

    model = vLLMServer(model_name)
    return model, model
...
def unload_model(keep_model_name=False):
    ...
    if model_class_name in ['Exllamav3Model', 'Exllamav3HF', 'TensorRTLLMModel']:
        shared.model.unload()
    elif model_class_name in ['LlamaServer', 'vLLMServer']:
        shared.model.stop()
